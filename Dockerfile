...
# Clonar Cardano Node
RUN cd /root && git clone https://github.com/IntersectMBO/cardano-node.git  && \
...
COPY scripts/build-cardano.sh /usr/local/bin/build-cardano.sh
RUN chmod +x /usr/local/bin/build-cardano.sh && /usr/local/bin/build-cardano.sh
# Script build-cardano.sh (compilación diferida)
# RUN  <<EOF > /usr/local/bin/build-cardano.sh
# #!/usr/bin/env bash
